  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <NavLink to="/" className="flex items-center gap-2.5 group" aria-label="Budget Bit home">
          <div
        ...
        <div className="flex items-center gap-1">
          {publicItems.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.to === "/"} className={linkCls} aria-label={item.label}>
              {item.icon}
              <span className="hidden sm:block">{item.label}</span>
            </NavLink>
          ))}

          {/* Upload Bill — only shown to logged-in users */}
          {profile && (
            <NavLink to="/upload" className={linkCls} aria-label="Upload Bill">
              <Upload size={17} />
              <span className="hidden sm:block">Upload Bill</span>
            </NavLink>
          )}
